...
c      PLHS(1) = MXCREATEDOUBLESCALAR(0.0)
      PLHS(1) = mxcreatenumericarray(1,1,mxClassIDFromClassName('int32'),0)
      do iii=2,NLHS
...
c      YPP = MXGETPR(PLHS(1))
      s_yeconsistent_p = MXGETPR(PLHS(1))
...
C       CREATED ARRAYS.  
C
      call enforce_ye_consistency(r_ye_inp, r_xnut, r_xprot, r_xalfa, r_xh, r_a, r_x, i_yeconsistent)
c      CALL YPRIME(RYPP,RTP,RYP)
C
c      write(*,*) 'mark6'
C COPY OUTPUT WHICH IS STORED IN LOCAL ARRAY TO MATRIX OUTPUT
c      CALL MXCOPYREAL8TOPTR(RYPP, YPP, 4)
      CALL MXCOPYINTEGER4TOPTR(i_yeconsistent, s_yeconsistent_p, 1)
